...
**Retour partiel (2026-06-22)** : un onglet « Données » a été réintroduit (`web/src/sections/Donnees.jsx`), mais d'une nature différente de celui retiré ci-dessus : pas de bac à sable de visualisation, un tableau réutilisable avec sélecteur de colonnes, filtres par colonne, tri, regroupement façon Power BI (catégorie choisie, effectif et moyennes par groupe) et export CSV qui respecte la vue courante. Librairie : `@tanstack/react-table` (headless, gratuite), habillage Tailwind avec les tokens existants. Demande explicite de Corentin, proposition validée avant codage.

**Refonte (2026-06-22, même jour)** : source de données changée de `respondents.json` (jeu recodé) vers le CSV brut (`reponses.csv`, via `papaparse`), pour disposer des 31 colonnes exactes du questionnaire, y compris les réponses libres Q19/Q20. Les colonnes ne sont plus une liste câblée en dur : chaque colonne du CSV est typée automatiquement (numérique / catégorielle / texte libre) selon ses valeurs. Le panneau de filtres et le sélecteur de colonnes sont passés dans une barre latérale gauche repliable plutôt qu'au-dessus du tableau, qui défile désormais horizontalement sans contrainte de largeur.
